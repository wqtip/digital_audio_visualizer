module butterfly#(
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            // 1. Perform complex multiplication
            BW_RE_reg <= (B_RE * W_RE) - (B_IM * W_IM);
            BW_IM_reg <= (B_RE * W_IM) + (B_IM * W_RE);
            
            // 2. Delay A to match the B*W latency
            A_RE_delayed <= A_RE;
            A_IM_delayed <= A_IM;
        end
    end
    ...
    // Scaling: extract the relevant bits from the 32-bit product
